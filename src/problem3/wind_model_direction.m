...
%t = @(p,X)(X(:,1).*sind(X(:,2)) +((X(:,1).*sind(X(:,2))).^2+2*p(2).*(p(1) - p(3)*X(:,3).*sind(X(:,5))./X(:,4)-p(4)*X(:,3).*cosd(X(:,5))./X(:,4))).^0.5)./(p(1) - p(3)*X(:,3).*sind(X(:,5))./X(:,4)-p(4)*X(:,3).*cosd(X(:,5))./X(:,4)); %时间量
...
%fun = @(p,X)(exp(-p(3)./(X(:,4)).*((X(:,1).*sind(X(:,2)) + ((X(:,1).*sind(X(:,2))).^2+2*p(1)*p(2)).^0.5)/p(1))).*(p(1)./(X(:,4)).*p(4)./(X(:,4)) + (p(3)./(X(:,4))).^2.*(X(:,3).*cosd(X(:,5))) - (p(4)./(X(:,4))).^2.*(X(:,3).*cosd(X(:,5))) - (p(3)./(X(:,4))).^2.*(X(:,1).*cosd(X(:,2))) - 2*p(3)./(X(:,4)).*p(4)./(X(:,4)).*(X(:,3).*sind(X(:,5))) + p(3)./(X(:,4)).*p(4)./(X(:,4)).*(X(:,1).*sind(X(:,2)))))./(p(3)./(X(:,4))).^3 - (p(1)*p(4)./(X(:,4)) + (p(3)./(X(:,4))).^2.*(X(:,3).*cosd(X(:,5))) - (p(4)./(X(:,4))).^2.*(X(:,3).*cosd(X(:,5))) - (p(3)./(X(:,4))).^2.*(X(:,1).*cosd(X(:,2))) - 2*p(3)./(X(:,4)).*p(4)./(X(:,4)).*(X(:,3).*sind(X(:,5))) + p(3)./(X(:,4)).*p(4)./(X(:,4)).*(X(:,1).*sind(X(:,2))))./(p(3)./(X(:,4))).^3 + (((X(:,1).*sind(X(:,2)) + ((X(:,1).*sind(X(:,2))).^2+2*p(1)*p(2)).^0.5)/p(1)).*(p(1)*p(4)./(X(:,4)) + (p(3)./(X(:,4))).^2.*(X(:,3).*cosd(X(:,5))) - (p(4)./(X(:,4))).^2.*(X(:,3).*cosd(X(:,5))) - 2*p(3)./(X(:,4)).*p(4)./(X(:,4)).*(X(:,3).*sind(X(:,5))) + p(3)./(X(:,4)).*p(4)./(X(:,4)).*(X(:,1).*sind(X(:,2)))))./(p(3)./(X(:,4))).^2 + (((X(:,1).*sind(X(:,2)) + ((X(:,1).*sind(X(:,2))).^2+2*p(1)*p(2)).^0.5)/p(1)).^2.*((p(4)./(X(:,4))).^2.*(X(:,3).*cosd(X(:,5))) - p(1)*p(4)./(X(:,4)) + p(3)./(X(:,4)).*p(4)./(X(:,4)).*(X(:,3).*sind(X(:,5)))))./(2*p(3)./(X(:,4)));
...
%参数设置(考虑空气升力时k为k/m,X(4)此时无用)
Pi = 3.14;
k0 = 0.5*0.53*1.293*((0.05)^2)*Pi;
p = [9.81,1.5,0.004,0.005];
v = 10;
a = 38;
m = 0.5;
...
line([c_theta(max_idx), c_theta(max_idx)], [11.4, max_val], 'LineStyle', '--', 'Color', 'k');
line([c_theta(min_idx), c_theta(min_idx)], [11.4, min_val], 'LineStyle', '--', 'Color', 'k');
