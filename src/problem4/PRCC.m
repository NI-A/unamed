...
% 定义目标函数
fun = @(p, X) X(:,1).*cosd(X(:,2)).*...
    ((X(:,1).*sind(X(:,2)) + ((X(:,1).*sind(X(:,2))).^2 + 2*p(2).*(p(1) - p(3)*X(:,3).*sind(X(:,5))./X(:,4))).^0.5) ./ ...
    (p(1) - p(3)*X(:,3).*sind(X(:,5))./X(:,4))) - ...
    p(3)*0.5./X(:,4).*(-X(:,3).*cosd(X(:,5)) + X(:,1).*cosd(X(:,2))).*...
    (((X(:,1).*sind(X(:,2)) + ((X(:,1).*sind(X(:,2))).^2 + 2*p(2).*(p(1) - p(3)*X(:,3).*sind(X(:,5))./X(:,4))).^0.5) ./ ...
    (p(1) - p(3)*X(:,3).*sind(X(:,5))./X(:,4))).^2);
...
bar(PRCC_values);
ylabel('PRCC 值');
xticklabels({'出手速度v_0(m/s)', '出手角度\theta(°)', '风速u(m/s)', '质量m(kg)', '风向\phi(°)'});
grid on;

% 在每个柱顶添加数值
for i = 1:length(PRCC_values)
    text(i, PRCC_values(i), num2str(PRCC_values(i), '%.4f'), ...
