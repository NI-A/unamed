...
% 初始化轨迹偏差矩阵
trajectory_diff = zeros(size(Velocity));
...
for i = 1:numel(Velocity)
    ...
    % 计算每一个x对应的y差值，评价轨迹偏差
    y_diff = abs(y_spin_interp - y_spinNo_interp);
    trajectory_diff(i) = mean(y_diff); % 使用平均差值作为轨迹偏差评价
end

% 绘制轨迹偏差热力图
figure;
imagesc(start_velocity, start_angle, trajectory_diff);
colorbar;
xlabel('Velocity (m/s)');
ylabel('Angle (°)');
...
saveas(gcf, 'Diff_Spin_NoApproximatetoApproximate.pdf');
