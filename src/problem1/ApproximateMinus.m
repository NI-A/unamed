...
% 初始化距离差值矩阵
distance_diff_approx_vs_nonapprox = zeros(size(Velocity));
...
for i = 1:numel(Velocity)
    ...
    % 计算距离差值
    distance_diff_approx_vs_nonapprox(i) = distance_approx - distance_nonapprox;
end

% 绘制热力图
figure;
imagesc(start_velocity, start_angle, distance_diff_approx_vs_nonapprox);
colorbar;
xlabel('Velocity (m/s)');
ylabel('Angle (°)');
